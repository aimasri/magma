...
* **Zero-Trust Cross-Domain SSO Handoff:** Authenticated users navigating between different tenant domains are securely and automatically redirected to their correct domain via short-lived SSO tokens generated through the `PersistentAuthenticationService` and evaluated by the `TenantSecurityMiddleware`.
