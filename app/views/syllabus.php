 ...
 * Teaching notes: Pure HTML representation of textbook.md
 */
...
> [!NOTE]
<br>
> <strong>Professor's Definitions:</strong>
<br>
> *   <strong>Dependency:</strong> An object that another object relies on to do its job. If your <code>ProductService</code> needs to save to a database, the <code>DatabaseConnection</code> is a dependency.
<br>
> *   <strong>Abstraction (Interface):</strong> A contract that defines *what* a class can do, without defining *how* it does it. Instead of depending on a specific <code>MySQLDatabase</code>, you depend on a generic <code>DatabaseInterface</code>.
<br>
> *   <strong>Static Method:</strong> A method that belongs to the class itself, not to a specific object instance. You can call it without using the <code>new</code> keyword (e.g., <code>Cache::get()</code>). Because it belongs to the class globally, it acts essentially like a global variable, which is dangerous for testability.
</p>
<p>
Modern frameworks often violate or obscure the Dependency Inversion principle for the sake of developer ergonomics.
</p>
<p>
> <strong>Historical Context:</strong> The popularization of "Magic" largely began with Ruby on Rails in the mid-2000s, which championed "Convention over Configuration." It assumed that if you named your database table <code>users</code> and your class <code>User</code>, it would dynamically wire everything together. Laravel brought this philosophy to PHP, heavily utilizing a pattern called <strong>Facades</strong>.
</p>
<p>
...
<p>
> <strong>Historical Context:</strong> Early PHP (often called "Spaghetti Code") famously mixed HTML, database connections, and business logic all in the same <code>.php</code> file. SoC was formalized to stop this madness, eventually giving rise to the MVC (Model-View-Controller) pattern.
</p>
<p>
...
<p>
#### File Walkthrough: <code>www/index.php</code>
<br>
...
<p>
> <strong>Q: What is the security risk of having the core application folder (e.g. <code>magma/</code>, which contains the <code>.env</code> file) inside the public <code>www/</code> directory?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: Looking at <code>index.php</code>, why does it make sense to attach "global" middleware (like CSRF and Session checks) here at the entrance, rather than waiting until the Router decides which Controller to execute?</strong>
<br>
>
<br>
...
<p>
#### File Walkthrough: <code>bootstrap.php</code>
</p>
<p>
...
<p>
> <strong>Q: Beyond saving us from typing <code>require</code> a thousand times, how does an Autoloader actually save memory and improve performance on a per-request basis?</strong>
<br>
>
<br>
> <strong>A:</strong> The autoloader only loads what is necessary (lazy loading). If an HTTP request only triggers 5 classes out of a 500-class application, the autoloader only reads and parses those 5 files into memory. If we manually <code>require</code>d everything upfront, we would waste massive amounts of RAM and CPU parsing unused code.
</p>
<p>
> <strong>Q: Why is it safer and more robust to load <code>.env</code> variables into a dedicated <code>Config</code> object during bootstrap, rather than letting developers write <code>$_ENV['DB_PASSWORD']</code> randomly inside their classes?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: What is the massive downside of hardcoding <code>$app = new Application(new Router(), new ErrorHandler());</code> directly inside <code>index.php</code> instead of using the Container? Is it memory bloat?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: If <code>$app->run()</code> is the trigger that actually processes the HTTP request, how does keeping the "bootstrap" phase separate from the "run" phase help us if we want to write a background cron job?</strong>
<br>
>
<br>
> <strong>A:</strong> It allows us to perform other tasks entirely! A cron job (like a script that emails users at midnight) doesn't involve an HTTP request. Because bootstrap and execution are separate, our cron script can simply <code>require 'bootstrap.php'</code> to get access to the database and services, and then execute its own specific code, entirely bypassing <code>$app->run()</code>.
</p>
<hr>
...
<p>
> <strong>Q: Why is it architecturally safer for a class to ask for its dependencies via its constructor (Injection) rather than instantiating them itself internally?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: If the DI Container didn't exist, what would be the main negative impact on our code when trying to instantiate a high-level class like a <code>Controller</code> that has deeply nested dependencies?</strong>
<br>
>
<br>
> <strong>A:</strong> We would have to write a massive amount of copied, nested <code>new</code> statements (e.g., <code>new Controller(new Service(new Repository(new Database())))</code>). This "manual wiring" is tedious, repetitive, and we would quickly get lost in the boilerplate code every time we needed to create an object.
</p>
<hr>
...
<p>
> <strong>Q: Why is our architectural rule of Strict Typing absolutely mandatory if we want to use an Auto-Wiring Container?</strong>
<br>
>
<br>
> <strong>A:</strong> It makes sure the reflection class understands exactly what it is looking for and doesn't get lost. If a constructor parameter is just <code>$service</code> without a type hint, Reflection only sees a <code>mixed</code> type. It has no idea what class to instantiate! The type hint is the physical map the Container follows.
</p>
<p>
> <strong>Q: Reflection forces PHP to analyze constructors while the application is running. What potential downside does this introduce every time a user makes a web request?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: If we bind <code>PaymentGatewayInterface</code> to <code>StripeGateway</code> in our Service Provider, and next year the business wants to switch to <code>PayPalGateway</code>, exactly what files in our core business logic (<code>OrderService</code>, <code>CheckoutController</code>) do we need to modify?</strong>
<br>
>
<br>
> <strong>A:</strong> Absolutely none of them! We only change the binding in the Service Provider to point to the new <code>PayPalGateway</code>. Because the core business logic only depends on the generic <code>PaymentGatewayInterface</code>, it remains entirely untouched. This is the Open/Closed Principle in action.
</p>
<p>
> <strong>Q: If Reflection is so smart, why can't it just automatically figure out which class to use when a constructor asks for an Interface?</strong>
<br>
>
<br>
> <strong>A:</strong> Because of intent and control. If we have 5 different classes that all implement <code>PaymentGatewayInterface</code> (Stripe, PayPal, ApplePay, FakeTestingGateway, etc.), it is physically impossible for PHP to magically guess which one the developer *intends* to use for this specific environment. By strictly mapping out the connections, we explicitly control exactly what logic is used and where.
</p>
<hr>
...
<p>
> <strong>Q: Imagine a junior developer adds a new route, but instead of just returning the mapped controller, they write code inside the Router class to query the database to check if the user is an admin. Why is this a severe architectural violation?</strong>
<br>
>
<br>
...
<p>
#### File Walkthrough: <code>SessionTimeoutMiddleware</code>
<br>
...
<p>
> <strong>Q: If a Middleware class detects a session timeout, it returns a Redirect response immediately without calling <code>$next</code>. Why is it highly efficient that the Middleware can abort the journey without ever letting the Request reach the Controller?</strong>
<br>
>
<br>
> <strong>A:</strong> The Controller does not need to know that everything is okay; it only needs to do its job. By letting the middleware block invalid requests, it saves the server from doing unnecessary work (like database queries or complex logic inside the controller) because the request is aborted early!
</p>
<p>
> <strong>Q: We described Middleware as an onion. What might be a real-world example of a Middleware that does its job by modifying the *Response* on the way out, rather than inspecting the *Request* on the way in?</strong>
<br>
>
<br>
...
<p>
#### File Walkthrough: The <code>Request</code> Object
<br>
...
<p>
#### File Walkthrough: The <code>Response</code> Object
<br>
...
<p>
> <strong>Q: How does requiring a Request object (rather than looking directly at <code>$_POST</code>) make writing automated unit tests for a Controller significantly easier and cleaner?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: If a controller uses <code>echo "Hello";</code>, the text is sent to the user instantly. If a controller instead returns <code>new Response("Hello");</code>, how does the "Middleware Onion" architecture benefit from this delay?</strong>
<br>
>
<br>
> <strong>A:</strong> It allows the framework to intercept the object on the way out! Because the output is delayed until the very end, we can perform any operation on the response before providing it to the user.
</p>
<hr>
...
> [!TIP]
<br>
> <strong>Performance Note:</strong> If you add a new route, you MUST run <code>php bin/cache_routes.php</code>. Because Magma reads from the compiled cache rather than the raw file, this ensures absolute maximum performance in production environments.
</p>
</div>
...
<p>
> <strong>Q: Imagine Sandbox Corp decides to launch a mobile app next year that communicates with our server via an API, not a web browser. If we had written all of our tax calculation and database logic directly inside the <code>CheckoutController</code>, why would building this new API be incredibly painful?</strong>
<br>
>
<br>
> <strong>A:</strong> Because we would have to rewrite and duplicate every single business rule (like calculating taxes) for the API! By keeping the Controller as just a "Traffic Cop" and pushing the logic into a Service, our new <code>ApiController</code> can simply inject the exact same <code>OrderService</code> and reuse 100% of the business logic.
</p>
<p>
> <strong>Q: In our example, the Controller asks the <code>OrderService</code> to do the work. If the <code>OrderService</code> discovers that a module is out of stock, should the <code>OrderService</code> be the one to generate the <code>new Response("Out of stock", 400)</code> object?</strong>
<br>
>
<br>
...
<p>
#### File Walkthrough: The <code>OrderService</code>
<br>
...
<p>
> <strong>Q: Imagine we wrote a command-line script to automatically generate 10 test orders for our developers. Why is it structurally impossible for that CLI script to use the <code>CheckoutController</code> to create the orders, and why does it *have* to use the <code>OrderService</code>?</strong>
<br>
>
<br>
> <strong>A:</strong> The Controller only deals with HTTP concepts; it requires an <code>HttpRequest</code> object and returns an <code>HttpResponse</code> object. A command-line script doesn't have an HTTP request! By using the <code>OrderService</code> directly, the CLI script bypasses the web layer entirely and utilizes the pure PHP logic.
</p>
<p>
> <strong>Q: The <code>OrderService</code> asks the <code>MailerInterface</code> to send an email. Why didn't we just write the <code>mail()</code> logic directly inside the <code>OrderService</code>?</strong>
<br>
>
<br>
> <strong>A:</strong> It violates the Single Responsibility Principle (SRP). The <code>OrderService</code> is responsible for orchestrating the order flow, not for dealing with SMTP protocols, email headers, and connection timeouts. By delegating to a <code>MailerInterface</code>, the service remains clean and focused.
</p>
<p>
> <strong>Q: Would <code>OrderService</code> eventually become monolithic and unwieldy? Eventually, will this class have to be extrapolated into multiple services that handle an order?</strong>
<br>
>
<br>
> <strong>A:</strong> Absolutely correct! This is a phenomenon known as the "God Class" anti-pattern. If <code>OrderService</code> grows to 2,000 lines because it handles taxes, fraud detection, shipping calculations, and inventory reservations, it violates SRP again.
<br>
...
<p>
> <strong>Q: Our <code>OrderService</code> requires an <code>OrderRepositoryInterface</code>. If we want to unit-test the <code>OrderService</code> without actually connecting to a real database, how does the Repository Pattern allow us to do that? Do we just write test SQL?</strong>
<br>
>
<br>
> <strong>A:</strong> We don't write any SQL to test it at all! Because the Service only requires the <code>Interface</code>, we can write a fake class (like an <code>InMemoryOrderRepository</code>) that simply returns hardcoded PHP arrays. We inject that fake class into our Service during the test, completely bypassing the database layer. The Service never knows the difference, and our tests run instantly.
</p>
<p>
> <strong>Q: If a new privacy law passes requiring us to "soft delete" orders (mark them as <code>deleted = 1</code> rather than actually dropping the row from the database), how many files in our codebase would we have to modify to ensure that <code>findById()</code> no longer returns deleted orders?</strong>
<br>
>
<br>
> <strong>A:</strong> Just one: The Repository! Because all SQL is quarantined inside the repository, we update the query there, and every Service and Controller across the entire application instantly respects the new privacy law.
</p>
<hr>
...
<p>
> <strong>Q: We write a terminal command that runs every night at midnight to generate a sales report for *all* vendors on the platform. If our Repositories are strictly locked to a single injected <code>TenantContext</code>, how would a single script generate reports for multiple different vendors?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: Some ORMs use "Global Scopes," where the framework magically intercepts the SQL behind the scenes and adds <code>WHERE vendor_id = 1</code> before it hits the database. Based on our philosophy in Module 1, why might we prefer explicitly writing the <code>WHERE</code> clause in the Repository over using a framework's magical global scope?</strong>
<br>
>
<br>
> <strong>A:</strong> You have to know what is going on behind the scenes to properly code new functions. Relying on "magic" can backfire entirely if you don't actually know where the data you are receiving is coming from, or if you encounter an edge case where you *need* to bypass the magic but can't figure out how. By explicitly injecting the context and writing the <code>WHERE</code> clause in the repository, the mechanism is completely transparent, readable, and debuggable.
</p>
<hr>
...
<p>
> <strong>Q: Imagine Sandbox Corp hires a junior Front-End Web Designer who only knows HTML and CSS, and does not know PHP. How does enforcing the "Dumb View" architecture make their job significantly safer and easier compared to the "Spaghetti Code" era?</strong>
<br>
>
<br>
> <strong>A:</strong> The data received is sanitized and empty of any logic that might harm or break the app. It provides simply the necessary information, ready for the view. The designer can just write HTML without worrying about accidentally deleting a database table or breaking complex backend routines.
</p>
<p>
> <strong>Q: The business wants to display a red "SALE!" badge next to any module that costs less than £10. A developer wants to write <code><?php if ($module->getPrice() < 10) { echo "<span class='sale'>SALE!</span>"; } ?></code> directly into the <code>index.html.php</code> View file. Why does this violate the "Dumb View" philosophy, and where *should* that "less than 10" logic actually live?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: If we didn't have a global Error Handler configured in <code>bootstrap.php</code>, and a developer forgot to write a <code>try/catch</code> block around a database query that failed, what are the *two* completely different ways the application might behave depending on the server's default configuration, and why are *both* terrible for a production site?</strong>
<br>
>
<br>
> <strong>A:</strong> It could show a full stack trace which looks ugly and presents a massive security risk (exposing file paths and potentially credentials), or it could show the White Screen of Death that tells the user absolutely nothing, providing a terrible User Experience (UX).
</p>
<hr>
</div>
...
<p>
> <strong>Q: Our <code>OrderRequestDTO</code> has a validation check: <code>if ($quantity <= 0) throw Exception;</code>. Why is it architecturally better to put this specific validation inside the DTO, rather than putting it inside the <code>OrderService</code> or the <code>CheckoutController</code>?</strong>
<br>
>
<br>
...
<p>
> <strong>Q: In "magical" frameworks like Laravel, you can configure the framework so that any time a database row is inserted, it *magically* and invisibly fires an event behind the scenes. Based on our philosophy of avoiding "Magic", why do we actively reject this, and instead force the developer to manually type <code>$this->dispatcher->dispatch(new OrderPlacedEvent($order))</code> inside the Service?</strong>
<br>
>
<br>
